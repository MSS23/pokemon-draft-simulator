...
export default function SpectateRoomPage() {
        ...
        <Card className="max-w-md">
          ...
          <CardContent>
            ...
            <div className="flex gap-2">
              <Button onClick={() => router.push('/spectate')} variant="outline">
  ...
  const currentPhase =
    draftState.draft.status === 'setup' ? 'setup' :
    draftState.draft.status === 'active' && draftState.draft.format === 'auction' ? 'auction' :
    draftState.draft.status === 'active' ? 'drafting' : 'completed'

  const currentTurn = draftState.draft.current_turn || 1
  const currentTeam = draftState.teams.find(t => {
    const teamOrder = Math.floor((currentTurn - 1) % draftState.teams.length) + 1
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="relative text-center mb-6">
          <div className="absolute top-0 left-0">
            <Button variant="ghost" size="sm" onClick={() => router.push('/spectate')}>
              <ArrowLeft className="h-4 w-4 mr-2" />
              Back
            </Button>
          </div>
          ...
          <div className="flex justify-center gap-2 mt-2">
            <Badge variant={draftState.draft.status === 'active' ? 'default' : 'secondary'}>
              {draftState.draft.status.charAt(0).toUpperCase() + draftState.draft.status.slice(1)}
            </Badge>
            <Badge variant="outline">
              {draftState.draft.format === 'snake' ? 'Snake Draft' : 'Auction Draft'}
            </Badge>
          </div>
        </div>
        ...
        {draftState.draft.status === 'active' && (
          <div className="mb-6">
            <DraftProgress
              ...
              draftStatus={draftState.draft.status === 'active' ? 'drafting' : 'waiting'}
              timeRemaining={60}
          ...
          {draftState.teams.map((team) => {
            ...
            return (
              <TeamRoster
                ...
                maxPokemonPerTeam={draftState.draft.settings?.maxPokemonPerTeam || 6}
              />
            )
          })}
        </div>
