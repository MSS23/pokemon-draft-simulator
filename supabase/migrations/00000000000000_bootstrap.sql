-- =====================================================================
-- BOOTSTRAP — base schema for FRESH Supabase projects only.
--
-- The live production database was created long before this file existed;
-- every change since lives in the timestamped migrations that follow.
-- This file (a snapshot of migrations/_archive/COMPLETE_SCHEMA.sql) exists
-- so a brand-new Supabase project can be built by running the migrations
-- folder in order — bootstrap first, then each timestamped migration.
--
-- It refuses to run against a database that already has the schema, so it
-- cannot half-apply over production (several statements here, e.g.
-- CREATE POLICY / CREATE TRIGGER, are not idempotent).
-- =====================================================================
DO $bootstrap_guard$ BEGIN
  IF to_regclass('public.drafts') IS NOT NULL THEN
    RAISE EXCEPTION 'Bootstrap is for fresh projects only — public.drafts already exists. Skip this file and apply only the timestamped migrations.';
  END IF;
END $bootstrap_guard$;
